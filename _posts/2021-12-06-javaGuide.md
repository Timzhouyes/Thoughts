...
> 注：此处java一次编译多处执行是在jvm后面，机器可执行的二进制机器码部分才是跟随机器的解释方式，比如32和64位可能不同的基本变量长度
>
> .class文件生成机器可执行的二进制码的过程，是在JVM之中的。所以JIT也是发生在JVM中，是JVM把热点代码进行保存。而AOT是在变异之中，把字节码编译成机器码，属于是替代了JVM的部分功能，避免了JIT预热等等开销。
...
> hash相关类的使用中都是先判断hashCode再判断equals，所以如果不重写hashCode的话会被直接拦截掉，重写的equals()方法无法生效
...
>
> 笔者：也就是“我能用到它，但是不因为我的持有而导致其不能被GC”
>
> 为什么要和对象的销毁相关？
>
...
3. 弱引用：弱引用之中，对象具有更短暂的生命周期，一旦发现**只有**弱引用的对象就会回收其内存。
...
避免类的**重复加载**，也保证Java 核心 API 不被篡改。比如我们自己写一个`java.lang.Object` 类，没有双亲委派模型，每个 classLoader 加载自己的类，就会出现很多个不同的 Object 类来干扰、
...
>
> 答：如果要使用二级缓存解决循环依赖，意味着所有Bean在实例化(doCreateBean())后就要完成AOP代理，这样违背了Spring设计的原则，Spring在设计之初就是通过`AnnotationAwareAspectJAutoProxyCreator`这个后置处理器来在Bean生命周期的最后一步来完成AOP代理，而不是在实例化后就立马进行AOP代理。
...
>
> 这实际上涉及到 AOP，如果创建的 Bean 是有代理的，那么注入的就应该是代理 Bean，而不是原始的 Bean。但是 Spring 一开始并不知道 Bean 是否会有循环依赖，通常情况下（没有循环依赖的情况下），Spring 都会在完成填充属性，并且执行完初始化方法之后再为其创建代理。但是，如果出现了循环依赖的话，Spring 就不得不为**其提前创建代理对象**(AOP之后的) ，否则注入的就是一个原始对象，而不是代理对象。因此，这里就涉及到应该在哪里提前创建代理对象？
>
...
2. @Autowired 是spring的注解，但是@Resource是java本身的注解
...
MyISAM 之中采用的是表级锁，InnoDB 之中支持行级锁和表级锁，默认是行级锁。
...
1. B 树的每一层里面，都是既有数据又有key，但是 B+树只有叶子节点存放 key 和 data，其他内存节点只存放 key。这个不同就让 B+树的查询时间更可控
...
复合索引，那么肯定不是主键索引，其就是非聚簇索引，里面存放的是在主键索引之中的地址。所以在查询复合索引时候，也是需要先拿到主键的聚簇索引的地址，然后再去主键索引之中找到对应的值。
...
**在一致性非锁定读下，**即使读取的记录已经被其他事务加上 X 锁，还是可以被继续读取，即其**快照数据**可以被接着读取。但是如果是**当前读**，那么每次读取的都是最新数据，如果两次查询之间有其他事务插入数据，就会产生幻读。因此 InnoDB 在实现可重复读的时候，执行当前读会对**读取的记录**加上 Next-key lock，防止其他事务在间隙间插入数据。
...
如果线程执行时间超过锁的过期时间怎么办？在这种情况下，线程 A 还在执行任务，线程 B 就获取到了锁，导致了 A和 B的冲突。之外， 如果这种情况下 A 已经执行完了任务，在打开锁的过程之中，是把B 的锁一起打开了。这时候 B 不仅和 A 冲突，自己上的锁还被打开了。
...
## 10.13 Redisson之中分布式锁的实现

参考：https://zhuanlan.zhihu.com/p/150602212

https://juejin.cn/post/6844904166322618376

Redisson之中分布式锁的实现，底层是使用Lua代码，Lua在Redis之中的作用也是保证**批量操作的原子性**。

### 底层原理

![img](../img/2021-12-06-javaGuide/v2-6a83e9a88942a6ec850a403640e46339_720w.jpg)

**需要注意：lua脚本在redis cluster之中是先选择一个节点，然后在这个master上面执行的！而不是整个集群广播！**

个人想法：本来也不需要广播，cluster之中的master是彼此独立的，key之间相互不干扰，当然不需要“集群层面“的处理。

### 加锁

![img](../img/2021-12-06-javaGuide/1723d128c41dce4c~tplv-t2oaga2asx-watermark.awebp)

1. 判断锁是否存在，如果不存在，设置值和过期时间，加锁成功
2. 锁存在：
   1. 锁的是否是当前线程，是的话，重入锁可以加锁成功，count+1. 释放时候也要释放2次
   2. 不是当前线程锁，返回过期时间

### 释放锁

![img](../img/2021-12-06-javaGuide/1723d128c52ba10e~tplv-t2oaga2asx-watermark.awebp)

1. 解锁线程和当前不是一个，解锁失败抛出异常
2. 解锁线程和当前线程同一个：
   1. Count-1,释放一次锁。如果剩余次数还是大于0，那么是可重入锁，刷新过期时间
   2. 剩余次数=0,publish 锁释放的消息，解锁成功

### 锁的数据结构

用redis之中的hash实现，map对象是锁的名字，key是**客户端的ID**，用来标示客户端；value是加锁次数。
...
> 
>
> ![img](../img/2021-12-06-javaGuide/v2-4d9e2740c823153bfc37bdf32935e263_720w.jpg)
>
> 
>
...
客户端的锁超过生存时间还想一直持有锁，怎么办？Redisson之中如果加锁成功，就会启动一个watch dog，每隔十秒检查一下，如果客户端还持有锁，会延长key的 续期时间。

### 缺点

还是主从切换部分。对某个redis cluster的master，如果客户端A加了锁，成功了，这时候准备复制给slave，但是突然宕机，slave变成了master，这个时候客户端B会在新的redis master上面完成加锁，那么客户端A也认为自己上了锁，就会有问题。

这个问题本质上不是redisson的问题，而是redis集群本身主从的问题。

### 如何解决：redLock

参考：https://smallpocket.github.io/2019/04/16/%E6%95%B0%E6%8D%AE%E5%BA%93/Redis%EF%BC%9A%E5%88%86%E5%B8%83%E5%BC%8F%E9%94%81/
...
>
> ![img](../img/2021-12-06-javaGuide/16652bd95e11a8b3)
>
