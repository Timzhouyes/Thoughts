...
插一句话：上面我们分析了在一个class之中有两个东西，一个是 constructor, 一个是 ngOnInit() 。 Angular 只能控制其中的 ngOnInit() ,也不知道这个是不是在教程之中强烈推荐将 getHeroes() 放到 ngOnInit() 之中使用而不是放在 constructor() 的原因，哈哈哈哈哈
